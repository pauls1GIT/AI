...
def tavily_search(query, include_images=False, max_results=5):
...
context_response = tavily_search("how to read technical drawings for bolts")
...
# =============================
# UPDATED PROMPT STYLE (ds.ipynb-inspired) — return AnswerBlock(...), not JSON
# =============================
...
# -----------------------------
# Helper: parse AnswerBlock(...) strings robustly
# -----------------------------

def parse_answerblock(text: str):
    """Extract (answer, confidence) from an AnswerBlock(...) string.
    Returns (answer:str, confidence:float). On failure, returns ("No value", 0.0).
    Accepts optional surrounding dicts like {'Answer': AnswerBlock(...)}.
    """
    if not text:
        return "No value", 0.0

    # Find the inner AnswerBlock(...)
    m_outer = re.search(r"AnswerBlock\s*\(\s*(.*?)\s*\)", text, re.DOTALL)
    if not m_outer:
        return "No value", 0.0
    inner = m_outer.group(1)

    # Extract answer='...'
    m_ans = re.search(r"answer\s*=\s*([\'\"])(.*?)\1", inner, re.DOTALL)
    answer_val = m_ans.group(2).strip() if m_ans else "No answer found"

    # Extract confidence=0.62
    m_conf = re.search(r"confidence\s*=\s*([0-9]*\.?[0-9]+)", inner)
    try:
        conf_val = float(m_conf.group(1)) if m_conf else 0.0
    except Exception:
        conf_val = 0.0

    return answer_val, conf_val
...
def analyze_bolt_image(image_path):
    ...
    for field in fields:
        ...
        # NEW: parse AnswerBlock output
        ans, conf = parse_answerblock(field_str)
        answers[field] = ans
...
# UPDATED to handle AnswerBlock format

def extract_answer(md_json):
    if not md_json:
        return "No value", 0.0
    return parse_answerblock(md_json)
